      ...
      <Tailwind>
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5">
            ...
            <Text className="text-sm leading-6 text-black">{t('domain-transferred-with-code', { "component0": {t('domain-transferred-with-code_component0', { "_domain_": <>{domain}</> })} })}
              {linksCount > 0 && (
                <>{t('links-count-message', { "linksCount": linksCount })}</>
              )}{t('workspace-domain-transfer-link', { "component0": <Link
                href={`https://app.dub.co/${newWorkspace.slug}/settings/domains`}
                className="font-medium text-blue-600 no-underline"
              >
                {t('workspace-domain-transfer-link_component0', { "newWorkspace_name": newWorkspace.name })}</Link> })}
