      ...
      <Tailwind>
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5">
            ...
            <Heading className="mx-0 my-7 p-0 text-center text-xl font-semibold text-black">{t('your-provider-links-have-been-imported-newline', { "provider": provider })}</Heading>
            <Text className="text-sm leading-6 text-black">{t('we-have-successfully-imported-links-from-provider', { "component0": <strong>{t('we-have-successfully-imported-links-from-provider_component0')}{Intl.NumberFormat("en-us").format(count)}{t('we-have-successfully-imported-links-from-provider_component0')}</strong>, "provider": provider, "component1": <Link
                href={`https://app.dub.co/${workspaceSlug}`}
                className="font-medium text-blue-600 no-underline"
              >
                {t('we-have-successfully-imported-links-from-provider_component1', { "workspaceName": workspaceName })}</Link> })}
              
              {pluralize("domain", domains.length)}{t('strong-domains-joined', { "component0": {t('strong-domains-joined_component0', { "_domains_join_": _domains_join_ })} })}
              </Text>
            {links.length > 0 && (
              <Section>
                <Row className="pb-2">
            ...
            {count > 5 && (
              <Section className="my-8 text-center">
                <Link
                  ...
                  href={`https://app.dub.co/${workspaceSlug}`}
                >{t('view')}{Intl.NumberFormat("en-us").format(count - 5)}{t('more-links')}</Link>
              </Section>
            )}
            <Text className="text-sm leading-6 text-black">{t('configured-your-domains', { "component0": <Link
                href="https://dub.co/help/article/how-to-add-custom-domain#step-2-configure-your-domain"
                className="font-medium text-blue-600 no-underline"
              >{t('configured-your-domains_component0')}{pluralize("domain", domains.length)}
              </Link> })}
              </Text>
            <Footer email={email} />
          </Container>
