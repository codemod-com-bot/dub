...
const t = useTranslations("../emails");
  ...
  return (
    <Html>
      <Head />
      <Preview>{t('your-dub-workspace-usage', { "name": name })}{percentage.toString()}{t('links-limit-percentage')}</Preview>
      <Tailwind>
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5">
            ...
            <Text className="text-sm leading-6 text-black">{t('workspace-links-usage-details', { "component0": {t('workspace-links-usage-details_component0', { "_strong_name_strong_": _strong_name_strong_ })}, "component1": <strong>{percentage.toString()}{t('workspace-links-usage-details_component1')}</strong> })}
              {capitalize(plan)}{t('total-links-created-info', { "component0": <strong>{nFormatter(linksUsage, { full: true })}{t('total-links-created-info_component0')}</strong> })}
              {nFormatter(linksLimit, { full: true })}{t('current-billing-cycle-links')}</Text>

            {plan === "business-max" || plan === "enterprise" ? (
              <Text className="text-sm leading-6 text-black">{t('current-plan-info')}{capitalize(plan)}{t('link-creation-after-limit')}</Text>
            ) : percentage === 100 ? (
              <Text className="text-sm leading-6 text-black">{t('existing-links-continue-working', { "component0": <Link
                  href={nextPlan.link}
                  className="font-medium text-blue-600 no-underline"
                >
                  {t('existing-links-continue-working_component0', { "nextPlan_name": nextPlan_name })}</Link> })}
                </Text>
            ) : (
              <Text className="text-sm leading-6 text-black">{t('upgrade-plan-after-limit', { "component0": <Link
                  ...
                  className="font-medium text-blue-600 no-underline"
                >
                  {t('upgrade-plan-after-limit_component0', { "nextPlan_name": nextPlan_name })}</Link> })}
                </Text>
            )}
