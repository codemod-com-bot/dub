    ...
    >
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 gap-5 sm:pt-20">
          <h1 className="text-4xl font-semibold">{t('application-header')}{hasPartnerProfile ? "submitted" : "saved"}
          </h1>
          <div className="flex flex-col gap-4 text-base text-neutral-700">
            {hasPartnerProfile && (
              <p>{t('application-submitted-message')}{application && (
                  <>
                    {t('application-update-email', { "component0": {t('application-update-email_component0', { "_application_email_": _application_email_ })} })}
                    </>
                )}
              </p>
            )}
            {!hasPartnerProfile && (
              <p>{t('account-setup-completion-message', { "component0": <strong className="font-semibold">{t('account-setup-completion-message_component0')}</strong> })}
                </p>
            )}
          </div>
        </div>
