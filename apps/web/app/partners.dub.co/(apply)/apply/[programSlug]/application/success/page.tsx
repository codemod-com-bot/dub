    ...
    >
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 gap-5 sm:pt-20">
          <h1 className="text-4xl font-semibold">{t('application')}{hasPartnerProfile ? "submitted" : "saved"}
          </h1>
          <div className="flex flex-col gap-4 text-base text-neutral-700">
            {hasPartnerProfile && (
              <p>{t('application-submitted-review')}{application && (
                  <>
                    {t('update-receive-email', { "component0": {t('update-receive-email_component0', { "_application_email_": _application_email_ })} })}
                    </>
                )}
              </p>
            )}
            {!hasPartnerProfile && (
              <p>{t('complete-account-setup', { "component0": <strong className="font-semibold">{t('complete-account-setup_component0')}</strong> })}
                </p>
            )}
          </div>
        </div>
