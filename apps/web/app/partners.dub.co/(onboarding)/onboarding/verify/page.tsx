...
const t = useTranslations("partners.dub.co/(onboarding)/onboarding/verify");
  ...
  return (
    <div className="mx-auto my-10 flex w-full max-w-sm flex-col items-center md:mt-14">
      ...
      <h1 className="animate-slide-up-fade mt-6 text-lg font-medium [--offset:8px] [animation-delay:250ms] [animation-duration:1s] [animation-fill-mode:both]">{t('setup-your-payouts-account')}</h1>
      <div className="animate-slide-up-fade mt-8 w-full [--offset:10px] [animation-delay:500ms] [animation-duration:1s] [animation-fill-mode:both]">
        <div className="divide-y divide-neutral-200 overflow-hidden rounded-lg border border-neutral-200">
          ...
          <div className="bg-white p-4 text-sm text-neutral-600">{t('we-use-stripe-to-ensure-you-get-paid-on-time', { "component0": <strong>{t('we-use-stripe-to-ensure-you-get-paid-on-time_component0')}</strong> })}
            <br />
            <br />{t('complete-at-later-date-no-payouts-until-completed')}</div>
        </div>
        <div className="mt-6 grid gap-4">
          <StripeConnectButton text={t('continue-to-stripe')} />
          <Link
            href="/programs"
            className="text-sm text-neutral-500 transition-colors enabled:hover:text-neutral-800"
          >{t('ill-complete-this-later')}</Link>
        </div>
      </div>
    </div>
  );
