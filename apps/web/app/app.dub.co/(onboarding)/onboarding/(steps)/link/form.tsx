...
const { t } = useTranslation("app.dub.co/(onboarding)/onboarding/(steps)/link");
    ...
    <>
      ...
      >
        <DestinationUrlInput
          domains={domains}
          right={
            <div className="animate-text-appear text-xs font-normal text-gray-500"><Trans
i18nKey="press-enter-to-submit"
components={{"0": <strong />}}
/></div>
          }
          {...register("url")}
        ...
        <div className="flex flex-col gap-2">
          <span className="block text-sm font-medium text-gray-700">{t('link-preview')}</span>
          <div className="relative aspect-[1.91/1] w-full overflow-hidden rounded-md border border-gray-300 bg-gray-100">
            {previewImage ? (
              ...
              <div className="relative flex size-full flex-col items-center justify-center space-y-4 bg-white">
                ...
                <p className="text-sm text-gray-400">{t('enter-a-link-to-generate-preview')}</p>
              </div>
            )}
            {loadingPreviewImage && (
