    ...
    <>
      ...
      >
        <DestinationUrlInput
          domains={domains}
          right={
            <div className="animate-text-appear text-xs font-normal text-gray-500">{t('press-enter-to-submit', { "component0": <strong>{t('press-enter-to-submit_component0')}</strong> })}</div>
          }
          {...register("url")}
        />
        <Controller
          ...
          <div className="relative aspect-[1.91/1] w-full overflow-hidden rounded-md border border-gray-300 bg-gray-100">
            {previewImage ? (
              ...
              <div className="relative flex size-full flex-col items-center justify-center space-y-4 bg-white">
                ...
                <p className="text-sm text-gray-400">{t('enter-a-link-to-generate-a-preview')}</p>
              </div>
            )}
            {loadingPreviewImage && (
