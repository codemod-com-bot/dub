...
const { t } = useTranslation("app.dub.co/(dashboard)/[slug]/settings/oauth-apps");
    ...
    <div className="grid gap-5">
      <div className="flex flex-wrap justify-between gap-6">
        <div className="flex items-center gap-x-2">
          <h1 className="text-2xl font-semibold tracking-tight text-black">{t('oauth-applications')}</h1>
          <InfoTooltip
            content={
              <TooltipContent
                title={t('learn-how-to-use-oauth-applications-to-build-integrations-with-dub')}
        ...
        {!isLoading ? (
          oAuthApps && oAuthApps.length > 0 ? (
            ...
            <div className="flex flex-col items-center gap-4 rounded-xl border border-gray-200 py-10">
              <EmptyState icon={Cube} title={t('no-oauth-applications-found')} />
            </div>
          )
        ) : (
          <div className="grid grid-cols-1 gap-3">
