    ...
    <div className="rounded-lg border border-neutral-200 bg-white">
      <div className="flex flex-col items-start justify-between gap-y-4 p-6 md:p-8 xl:flex-row">
        <div>
          ...
          <p className="mt-1 text-balance text-sm leading-normal text-neutral-500">{t('you-are-currently-on-the')}
            {plan ? (
              <PlanBadge plan={plan} />
            ) : (
              <span className="rounded-full bg-neutral-200 px-2 py-0.5 text-xs text-neutral-200">{t('load')}</span>
            )}{t('current-plan')}{billingStart && billingEnd && (
              <>
                {t('current-billing-cycle', { "component0": <span className="font-medium text-black">
                  {t('current-billing-cycle_component0', { "billingStart": billingStart, "billingEnd": billingEnd })}
                </span> })}
                </>
            )}
          </p>
