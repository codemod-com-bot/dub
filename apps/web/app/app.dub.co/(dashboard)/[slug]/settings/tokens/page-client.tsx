...
const t = useTranslations("app.dub.co/(dashboard)/[slug]/settings/tokens");
  ...
  const { table, ...tableProps } = useTable({
    ...
    emptyState: (
      <AnimatedEmptyState
        ...
        description={t('no-tokens-created-workspace-yet')}
        cardContent={() => (
          <>
  ...
  return (
    <div className="grid grid-cols-1">
      ...
      <p className="mb-2 mt-2 text-base text-neutral-600">{t('api-keys-access-workspace', { "component0": <a
          href="https://dub.co/docs/api-reference/tokens"
          target="_blank"
          className="font-medium underline underline-offset-4 hover:text-black"
        >{t('api-keys-access-workspace_component0')}</a> })}
      ...
      {tokens?.length !== 0 ? (
        ...
        <AnimatedEmptyState
          ...
          description={t('no-tokens-created-workspace-yet-duplicate')}
          cardContent={() => (
            <>
              <Key className="size-4 text-neutral-700" />
              <div className="h-2.5 w-24 min-w-0 rounded-sm bg-neutral-200" />
