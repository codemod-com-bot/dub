  ...
  return (
    ...
    >
      ...
      >
        ...
        >{t('welcome-to-app-name', { "process_env_NEXT_PUBLIC_APP_NAME": process.env.NEXT_PUBLIC_APP_NAME })}
        </motion.h1>
        <motion.p
          className="max-w-xl text-gray-600 transition-colors sm:text-lg"
          variants={STAGGER_CHILD_VARIANTS}
        >
          {t('custom-domain-link-management', { "component0": {t('custom-domain-link-management_component0', { "_domain_": <>{domain}</> })}, "component1": {t('custom-domain-link-management_component1', { "_process_env_NEXT_PUBLIC_APP_NAME_": <>{process.env.NEXT_PUBLIC_APP_NAME}</> })} })}
          </motion.p>
        <motion.a
          variants={STAGGER_CHILD_VARIANTS}
        ...
        >{t('create-free-branded-link')}</motion.a>
      </motion.div>
    </motion.div>
  );
}
