            ...
            <div className="flex items-center gap-3">
              {getValue() ? (
                <>
                  ...
                  <span className="truncate" title={t('qr-scan')}>{t('qr-scan-with-whitespace')}</span>
                </>
              ) : (
                <>
                  <CursorRays className="size-4 shrink-0" />
                  <span className="truncate" title={t('link-click')}>{t('link-click-with-whitespace')}</span>
                </>
              )}
            </div>
          ),
          ...
          cell: ({ getValue }) =>
            getValue() ? (
              ...
              <Tooltip content="We do not record IP addresses for EU users.">
                <span className="cursor-default truncate underline decoration-dotted">{t('unknown-status')}</span>
              </Tooltip>
            ),
        },
        // Sale amount
        {
          ...
          cell: ({ getValue }) => (
            <div className="flex items-center gap-2">
              ...
              <span className="text-neutral-400">{t('usd-currency')}</span>
            </div>
          ),
        },
        // Sale invoice ID
