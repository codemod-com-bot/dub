    ...
    >
      <div className="relative mt-1 rounded-md shadow-sm">
        <input
          ...
          name="search"
          id="search"
          placeholder={t('search-for-an-image-placeholder')}
          autoFocus={!isMobile}
          autoComplete="off"
          value={search}
      ...
      {data ? (
        data.length > 0 ? (
          <div className="mt-3 grid grid-cols-2 gap-3">
            {data.map((photo) => (
              ...
              >
                ...
                />
                <p className="absolute bottom-0 left-0 right-0 line-clamp-1 w-full bg-black bg-opacity-10 p-1 text-xs text-white">{t('image-author-link', { "component0": {t('image-author-link_component0', { "_photo_user_name_": <>{photo.user.name}</> })} })}
          ...
          <div className="flex h-[90%] items-center justify-center">
            <p className="text-center text-sm text-gray-500">{t('no-results-found-message')}<br />{t('suggestion-to-tweak-search-query')}</p>
          </div>
        )
      ) : (
        <div className="flex h-[90%] items-center justify-center">
