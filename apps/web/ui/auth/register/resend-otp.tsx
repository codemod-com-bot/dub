...
const { t } = useTranslation("../ui/auth/register");
    ...
    <div className="relative mt-4 text-center text-sm text-gray-500">
      {state === "default" && (
        <>
          ...
          <p className={cn(isExecuting && "opacity-80")}>{t('didnt-receive-a-code')}
            <button
      ...
      {state === "success" && (
        <p className="text-sm text-gray-500">{t('code-sent-successfully')}<Delay seconds={delaySeconds} />
        </p>
      )}

      {state === "error" && (
        <p className="text-sm text-gray-500">{t('failed-to-send-code')}<Delay seconds={delaySeconds} />
        </p>
      )}
    </div>
  );
...
const { t } = useTranslation("../ui/auth/register");
  ...
  return (
    <span className="ml-1 text-sm tabular-nums text-gray-400">{t('seconds-s', { seconds })}</span>
  );
};
